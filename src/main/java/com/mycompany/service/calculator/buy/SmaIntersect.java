 ...
 * @author setu
 */
public class SmaIntersect extends BuySignalCalculator {

    public SmaIntersect(ScannerService scanner, CustomHashMap oneYearData, Portfolio portfolio) {
    ...
    public boolean isBuyCandidate(List<Item> itemSubList, Item calculated) {
        ...
        if (
                (todayGap >= 4 && todayClosePrice>sma25 && today.getOpenPrice()<sma10 && todaysmaDiff>=1)
                && divergence <= -5
                && todayValue >= minValue
                && todayTrade >= minTrade
                && vChange >= 2
